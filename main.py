    ...
    if saved:
        ...
        await update.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("👔 Режим HR — найти кандидатов", callback_data="hr_search_start")],
            ])
        )
    else:
        await update.message.reply_text(
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Сохраняем пользователя и резюме в БД
        await db.upsert_user(user.id, user.username, user.full_name)
        embedding = pipeline.embed_resume(resume).tolist()
        ...
        logging.error(f"Ошибка обработки PDF: {e}", exc_info=True)
...
def _edit_or_search_keyboard(subscribed: bool = False) -> InlineKeyboardMarkup:
    sub_btn = (
        ...
        if subscribed else
        InlineKeyboardButton("🔕 Включить подписку", callback_data="sub_toggle")
    )
...
async def show_search_filters(message, user_id: int, context):
    ...
    experience = prefs.get('experience') or ['noExperience', 'between1And3', 'between3And6', 'moreThan6']
    ...
    await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            ...
            [
                ...
                InlineKeyboardButton(f"🙈 Без зарплаты: {'ВКЛ' if show_no_salary else 'ВЫКЛ'}",
...
# Специальный ID 0 = Remotive (международная удалёнка)
POPULAR_AREAS = [
    (113,  "Вся Россия"),
    (1,    "Москва"),
    (2,    "Санкт-Петербург"),
...
async def _show_location_picker(message, context):
    selected = context.user_data.get('filter_areas_selected',
    ...
    # Кастомные города — показываем отдельно с крестиком
    for area_id, name in custom.items():
        if area_id not in dict(POPULAR_AREAS):
            mark = "✅" if area_id in selected else "⬜"
            buttons.append([InlineKeyboardButton(
                f"{mark} {name} ✕", callback_data=f"area_{area_id}"
            )])
...
async def callback_area_toggle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for cid, cname in custom.items():
        if cid not in dict(POPULAR_AREAS):
            ...
            buttons.append([InlineKeyboardButton(
                f"{mark} {cname} ✕", callback_data=f"area_{cid}"
            )])
...
async def callback_area_reset(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сбрасываем и в prefs чтобы не подгружались из БД
    prefs = context.user_data.get('prefs', {})
    prefs['areas'] = []
    prefs['area_names'] = []
    context.user_data['prefs'] = prefs
...
async def callback_exp_toggle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prefs = context.user_data.get('prefs', {})
    selected = list(prefs.get('experience') or
...
async def callback_do_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Параллельные запросы по регионам и опыту
    all_vacancies = []
    experience = prefs.get('experience') or ['noExperience', 'between1And3', 'between3And6', 'moreThan6']
    show_without_salary = prefs.get('show_without_salary', True)

    # Разделяем hh.ru и Remotive
    hh_areas = [a for a in areas if a != 0]
    use_remotive = 0 in areas
    logging.debug(f"Поиск: areas={areas}, hh_areas={hh_areas}, use_remotive={use_remotive}")
    ...
    for v in all_vacancies:
        ...
        if vid not in seen:
            seen.add(vid)
            # Пост-фильтр по верхней границе зарплаты
            if salary_to:
                s = v.get('salary')
                if s:
    ...
    if resume:
        resume_vector = pipeline.embed_resume(resume)
    else:
        saved = await db.get_resume(user.id)
    ...
    await _send_matches(query.message, matches, user_id=user.id, context=context, total=len(matches))
...
async def handle_filter_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if context.user_data.get('awaiting_custom_city'):
        ...
        # Показываем до 5 вариантов
        buttons = []
        for item in results[:5]:
            area_id = int(item['id'])
            name = item['text']
            buttons.append([InlineKeyboardButton(
                name, callback_data=f"addarea_{area_id}_{name[:20]}"
            )])
...
async def handle_edit_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == EDIT_POSITION:
        ...
        # Пересохраняем обновлённое резюме в БД
        try:
            embedding = pipeline.embed_resume(resume).tolist()
            resume_id = await db.save_resume(user.id, resume, embedding)
            context.user_data['resume_id'] = resume_id
        except Exception as e:
            logging.error(f"Ошибка сохранения резюме: {e}")

        await update.message.reply_text(
            f"✅ Данные обновлены!\n\n{resume.summary()}",
            reply_markup=_edit_or_search_keyboard(subscribed=bool(await db.get_subscription(user.id)))
        )
...
async def run_matching(message, resume, user_id: int = None, context=None):
    ...
    try:
        ...
        logging.error(f"Ошибка матчинга: {e}", exc_info=True)
...
async def run_matching_from_db(message, saved_resume: dict, user_id: int = None, context=None):
    ...
    try:
        ...
        logging.error(f"Ошибка матчинга из БД: {e}", exc_info=True)
        await message.reply_text(f"❌ Ошибка: {str(e)}")


async def _send_matches(message, matches, user_id: int = None, context=None, offset: int = 0, total: int = None):
    if not matches:
    ...
    # Кнопка "Показать ещё" если есть что показывать
...
async def handle_hh_url(update: Update, context: ContextTypes.DEFAULT_TYPE, url: str):
    ...
    try:
        ...
        logging.error(f"Ошибка парсинга hh.ru: {e}")
        await update.message.reply_text(f"❌ Ошибка: {str(e)}")
...
async def fetch_vacancies(query: str, area: int = 1, limit: int = 500, **kwargs) -> list[dict]:
    """Получает вакансии с hh.ru с пагинацией. limit — максимальное кол-во."""
    per_page = 100
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            for page in range(max_pages):
                ...
                # Если страниц меньше чем запросили — выходим
...
async def callback_show_myresume(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    # Создаём фейковый update с message для переиспользования my_resume
...
def _myresume_keyboard(subscribed: bool = False) -> InlineKeyboardMarkup:
    sub_btn = (
        InlineKeyboardButton("🔔 Подписка: ВКЛ", callback_data="sub_toggle")
        if subscribed else
        InlineKeyboardButton("🔕 Подписка: ВЫКЛ", callback_data="sub_toggle")
    )
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("✏️ Редактировать", callback_data="edit_start"),
...
async def callback_confirm_delete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logging.error(f"Ошибка удаления резюме: {e}")
...
async def subscribe(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Начинаем настройку — выбор частоты
...
async def callback_freq(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if freq == 'weekly':
        # Для "по дням недели" — показываем выбор дней
        context.user_data['sub_days'] = []
        await _ask_days(query.message, context)
    else:
        # Для остальных — сразу спрашиваем время
...
async def handle_sub_time_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """
    Обрабатывает ввод времени для подписки.
    Возвращает True если обработал, False если не наш ввод.
    """
...
async def send_digest_tick(context: ContextTypes.DEFAULT_TYPE):
    ...
    Проверяет у каких подписчиков сейчас время рассылки и отправляет.
    """
    import json
    import numpy as np
    ...
    current_weekday = now.isoweekday()  # 1=пн, 7=вс
    current_day = now.day
    ...
    for sub in subscriptions:
        try:
            ...
            # Проверяем совпадение времени (с точностью до 30 минут)
            if abs(current_hour * 60 + current_minute - send_hour * 60 - send_minute) > 15:
                continue

            # Проверяем день
            if freq == 'daily' or freq == 'twice_daily':
                pass  # каждый день
            elif freq == 'weekly':
            ...
            logger.error(f"Ошибка рассылки для {sub.get('user_id')}: {e}")


async def callback_favorite(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Кнопка ⭐️ под вакансией — добавить/убрать из избранного"""
    query = update.callback_query
    user = update.effective_user
...
async def _send_favorites(message, user_id: int):
    ...
    for i, fav in enumerate(favs, 1):
        ...
        await message.reply_text(
            ...
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🗑 Удалить", callback_data=f"fav_{fav['vacancy_id'][:50]}")
            ]])
        )
...
async def callback_show_favorites(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    user = update.effective_user
    await _send_favorites(query.message, user.id)
...
async def fetch_remoteok(query: str, limit: int = 100) -> list[dict]:
    ...
    logging.info(f"RemoteOK: query='{query}' → tag='{tag}'")

    try:
        async with httpx.AsyncClient(timeout=15, headers={'User-Agent': 'JobBot/1.0'}) as client:
            response = await client.get(f"https://remoteok.com/api?tag={tag}")
            response.raise_for_status()
            data = response.json()
            jobs = [j for j in data if isinstance(j, dict) and j.get('id')]

        logging.info(f"RemoteOK вернул {len(jobs)} вакансий (tag={tag})")

        normalized = []
        for job in jobs[:limit]:
            salary_text = job.get('salary', '') or ''
            desc = _re.sub(r'<[^>]+>', ' ', job.get('description', '') or '')[:300]
            normalized.append({
                'id': f"remoteok_{job['id']}",
                'name': job.get('position', ''),
                'employer': {'name': job.get('company', '')},
                'alternate_url': job.get('url', f"https://remoteok.com/remote-jobs/{job['id']}"),
                'salary': None,
        ...
        logging.error(f"Ошибка RemoteOK: {e}")
        return []
...
async def fetch_hh_resumes(vacancy_text: str, limit: int = 20) -> list[dict]:
    ...
    if not token:
        logging.warning("HH_EMPLOYER_TOKEN не задан — пропускаем hh.ru резюме")
        return []
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            ...
            if response.status_code == 403:
                logging.warning("hh.ru: нет доступа к резюме (нужен платный аккаунт работодателя)")
                return []
            response.raise_for_status()
            items = response.json().get('items', [])
            logging.info(f"hh.ru резюме: найдено {len(items)}")
            return items
    except Exception as e:
        logging.error(f"Ошибка поиска резюме hh.ru: {e}")
        return []
...
async def handle_hr_vacancy_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    vacancy_vector = pipeline.model.encode(vacancy_text, normalize_embeddings=True)

    # Параллельно грузим нашу БД и hh.ru
    ...
    for r in our_resumes:
        ...
        try:
            ...
            score = float(np.dot(vacancy_vector, resume_vec) /
    ...
    for item in hh_items:
        ...
        # Эмбеддим заголовок + навыки из hh
        skills_hh = [s.get('name', '') for s in item.get('skill_set', [])]
        text_for_embed = f"{title} {' '.join(skills_hh)}"
        try:
            hh_vec = pipeline.model.encode(text_for_embed, normalize_embeddings=True)
            score = float(np.dot(vacancy_vector, hh_vec) /
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(callback_show_more,         pattern="^more_\\d+$"))
    ...
    app.add_handler(CallbackQueryHandler(callback_filter_location,        pattern="^filter_location$"))
    app.add_handler(CallbackQueryHandler(callback_filter_salary,          pattern="^filter_salary$"))
    app.add_handler(CallbackQueryHandler(callback_filter_experience,      pattern="^filter_experience$"))
    app.add_handler(CallbackQueryHandler(callback_filter_toggle_no_salary,pattern="^filter_toggle_no_salary$"))
    app.add_handler(CallbackQueryHandler(callback_do_search,              pattern="^do_search$"))
    app.add_handler(CallbackQueryHandler(callback_area_toggle,            pattern="^area_\\d+$"))
    ...
    app.add_handler(CallbackQueryHandler(callback_day_toggle,     pattern="^day_\\d+$"))
    ...
    # Проверяем подписки каждые 30 минут
